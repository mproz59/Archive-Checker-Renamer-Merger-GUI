...
TRANSLATIONS = {
    "FR": {
        ...
        "btn_select": "📁 Sélectionner un Dossier",
        "btn_stop": "⏹️ Arrêter",
        "btn_toggle_all_check": "☑️ Tout cocher",
        "btn_toggle_all_uncheck": "🔳 Tout décocher",
        "btn_rename": "🏷️ Formater en 0001 (Sélection)",
        "btn_merge": "⚡ Fusionner la sélection",
        "lbl_folder_placeholder": "Glissez-déposez un dossier ici ou cliquez sur Sélectionner",
        "lbl_dest": "📂 Dossier de destination (Fusion) :",
        ...
        "lbl_details_title": "Détails de l'archive sélectionnée :",
        "yes": "OUI ✅",
        "no": "NON ❌",
        ...
        "about_title": f"À propos - v{APP_VERSION}",
        "about_btn": f"ℹ️ v{APP_VERSION}",
        "about_content": (
            "📖 FONCTIONNEMENT DE L'APPLICATION\n"
            "--------------------------------------------------\n"
            "• Vérification Sécurité & Séquence : Détecte les fichiers indésirables (.bin)\n"
            "  et contrôle la continuité numérique des fichiers.\n"
            "• Colonne Format 0000 : Indique si l'archive respecte le formatage sur 4 chiffres.\n"
            "• Garde-fou de Fusion : Seules les archives formatées sur 4 digits peuvent être fusionnées.\n"
            "• Formater en 0001 : S'applique uniquement aux archives cochées dans la sélection.\n"
            "• Nommage automatique CBZ : 'DOSSIER - XXXX to XXXX.cbz'.\n\n"
            "--------------------------------------------------\n"
            "HISTORIQUE DES VERSIONS (CHANGELOG)\n"
            "--------------------------------------------------\n"
            "📌 Version 1.4 :\n"
            "   - Amélioration de la stabilité générale du multithreading.\n"
            "   - Consolidation du système multilingue FR/EN.\n"
            "   - Optimisation des regex de détection des numéros décimaux (ex: 3.5).\n\n"
            "📌 Version 1.3 :\n"
            "   - Support multilingue (FR / EN).\n"
            "   - Garde-fou à la fusion si le format 0000 n'est pas appliqué.\n"
            "   - Colonne d'état 'Format 0000'.\n"
            "   - Renommage ciblé strictement sur les éléments cochés.\n\n"
            "📌 Version 1.2 :\n"
            "   - Support des numéros décimaux (ex: chapitre 3.5 -> 0003.5).\n"
            "   - Nommage automatique direct du CBZ : 'DOSSIER - XXXX to XXXX.cbz'.\n"
            "   - Lancement direct de la fusion sans étape 'Enregistrer sous'.\n\n"
            "📌 Version 1.1 :\n"
            "   - Ajout du bouton 'Tout cocher / Décocher' (adapté au filtre).\n"
            "   - Prise en charge du Glisser-Déposer (Drag & Drop) de dossiers.\n"
            "   - Gestion du dossier de destination automatique ou verrouillé.\n"
            "   - Règle de sécurité : Ignores les fichiers sans numéros lors du renommage.\n\n"
            "📌 Version 1.0 :\n"
            "   - Analyse récursive des dossiers et sous-dossiers (.zip, .cbz, .rar, .7z).\n"
            "   - Interface graphique PyQt6 avec filtrage en temps réel.\n"
        )
    },
    "EN": {
        ...
        "btn_rename": "🏷️ Format to 0001 (Selection)",
        "btn_merge": "⚡ Merge Selection",
        "lbl_folder_placeholder": "Drag and drop a folder here or click Select",
        "lbl_dest": "📂 Destination Folder (Merge):",
        ...
        "col_range": "Sequence",
        "col_bin": ".BIN Files",
        "lbl_details_title": "Details of selected archive:",
        "yes": "YES ✅",
        "no": "NO ❌",
        ...
        "about_title": f"About - v{APP_VERSION}",
        "about_btn": f"ℹ️ v{APP_VERSION}",
        "about_content": (
            "📖 APPLICATION OVERVIEW\n"
            "--------------------------------------------------\n"
            "• Security & Sequence Check: Detects unwanted files (.bin) and checks numeric continuity.\n"
            "• 0000 Format Column: Indicates if the archive follows 4-digit formatting.\n"
            "• Merge Guardrail: Only 4-digit formatted archives can be merged.\n"
            "• Format to 0001: Applies strictly to checked archives in the selection.\n"
            "• Automatic CBZ Naming: 'FOLDER - XXXX to XXXX.cbz'.\n\n"
            "--------------------------------------------------\n"
            "VERSION HISTORY (CHANGELOG)\n"
            "--------------------------------------------------\n"
            "📌 Version 1.4:\n"
            "   - General multithreading stability improvements.\n"
            "   - Multilingual FR/EN system consolidation.\n"
            "   - Optimized regex parsing for decimal numbers (e.g. 3.5).\n\n"
            "📌 Version 1.3:\n"
            "   - Multilingual support (FR / EN).\n"
            "   - Merge guardrail if 0000 format is not applied.\n"
            "   - Added '0000 Format' status column.\n"
            "   - Renaming strictly targets checked items.\n\n"
            "📌 Version 1.2:\n"
            "   - Support for decimal numbers (e.g. chapter 3.5 -> 0003.5).\n"
            "   - Direct automatic CBZ naming: 'FOLDER - XXXX to XXXX.cbz'.\n"
            "   - Direct merge without 'Save As' dialog step.\n\n"
            "📌 Version 1.1:\n"
            "   - Added 'Check All / Uncheck All' button.\n"
            "   - Drag & Drop folder support.\n"
            "   - Auto or locked destination folder management.\n"
            "   - Safety rule: Ignores files without numbers during renaming.\n\n"
            "📌 Version 1.0:\n"
            "   - Recursive scan for .zip, .cbz, .rar, .7z archives.\n"
            "   - Real-time PyQt6 GUI filtering.\n"
        )
    }
}
...
class ArchiveWorker(QThread):
    ...
    def analyze_archive(self, archive_path):
        ...
        try:
            ...
            if bin_files:
                has_bin = True
                details.append(f"❌ {len(bin_files)} .BIN file(s) found : {', '.join(bin_files)}")
...
class ArchiveCheckerApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.btn_select.setFont(QFont("Arial", 10, QFont.Weight.Bold))
        ...
        self.btn_toggle_all.setFixedHeight(40)
        self.btn_toggle_all.setFont(QFont("Arial", 10))
        self.btn_toggle_all.clicked.connect(self.toggle_all_checkboxes)

        self.btn_rename = QPushButton()
        self.btn_rename.setFixedHeight(40)
        self.btn_rename.setFont(QFont("Arial", 10, QFont.Weight.Bold))
        ...
        self.btn_merge.setFont(QFont("Arial", 10, QFont.Weight.Bold))
        ...
        self.btn_about.setFont(QFont("Arial", 10, QFont.Weight.Bold))
        ...
        self.btn_lang.setFont(QFont("Arial", 10, QFont.Weight.Bold))
        self.btn_lang.setStyleSheet("background-color: #6a1b9a; color: white;")
        self.btn_lang.clicked.connect(self.toggle_language)

        self.lbl_folder = QLabel()
        self.lbl_folder.setFont(QFont("Arial", 10))
        self.lbl_folder.setStyleSheet("color: #666; margin-left: 10px; font-style: italic;")
        ...
        self.lbl_details_title.setFont(QFont("Arial", 10, QFont.Weight.Bold))
    ...
    def update_language_ui(self):
        ...
        if not self.current_folder:
            self.lbl_folder.setText(t["lbl_folder_placeholder"])
        ...
        for row in range(self.table.rowCount()):
            ...
            if item_bin:
                item_bin.setText(t["yes"] if res["has_bin"] else t["no"])
    ...
    def set_root_folder(self, folder):
        ...
        self.lbl_folder.setStyleSheet("color: #000; font-style: normal;")
    ...
    def rename_selected(self):
        ...
        for row in range(self.table.rowCount()):
            ...
            if item and item.checkState() == Qt.CheckState.Checked:
                selected_paths.append(self.results_data[row]["path"])
    ...
    def add_result(self, res):
        ...
        item_count = QTableWidgetItem(f"{res['file_count']}")
        item_range = QTableWidgetItem(res["range"])
        item_bin = QTableWidgetItem(t["yes"] if res["has_bin"] else t["no"])
    ...
    def merge_selected(self):
        ...
        for row in range(self.table.rowCount()):
            item = self.table.item(row, 0)
            if item and item.checkState() == Qt.CheckState.Checked:
                selected_items.append(self.results_data[row])
